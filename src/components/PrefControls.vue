...
/** 按当前选择的湖北地区同时刷新天气和菜价。 */
export async function refreshSelectedLocation() {
  await refreshLocationData(city.value, city.value.name)
}

async function refreshLocationData(coords, label) {
  await Promise.allSettled([refreshWeather(coords, label), refreshPrices()])
}
...
export function setCity(id) {
  ...
  persist()
  // 换地区后同时刷新天气和菜价；菜价接口当前返回全国估算，城市选择用于后续本地数据源扩展。
  void refreshSelectedLocation()
}

/** 首次进入时并行拉取菜价和当前位置天气；定位失败时仅保留手动入口。 */
